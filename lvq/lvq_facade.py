...
class Lvq_facade:
    ...
    def start_process(self):
        ...
        P_train, T_train, test_P, test_T = train_test_split(P, T, test_size=0.10)
        
        learning_rate_iter = np.arange(0.01,0.1,0.005).tolist()
        codebooks_number_iter = np.arange(5,305,5).tolist()
        
        print(len(learning_rate_iter))
        print(len(codebooks_number_iter))
        
        try:
            lvq1_net = Lvq1([5,5], [0,1], epochs = 300)
            ...
            print("Something went wrong with file generation", e)
        else:
            csvfile.close()
